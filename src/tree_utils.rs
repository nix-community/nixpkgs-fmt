...
use rnix::{
    SyntaxKind::TOKEN_WHITESPACE, SyntaxElement, SyntaxNode, SyntaxToken, WalkEvent, NodeOrToken,
};
...
pub(crate) fn walk_non_whitespace(
    node: &SyntaxNode,
...
pub(crate) fn preceding_tokens<'a>(node: &'a SyntaxNode) -> impl Iterator<Item = SyntaxToken> {
